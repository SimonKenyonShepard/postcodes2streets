...
const osmOutputDirectory = 'osmOutput';
...
const updatePostcodeFiles = (postCodeData) => {
  ...
  for (const [postcode, streetNamesSet] of Object.entries(postCodeData)) {
    const outputFilePath = path.join(osmOutputDirectory, `${postcode}`);
    const finalOutputPath = path.join(finalOutputDirectory, `${postcode}`);

    let existingStreetNamesSet = new Set();

    if (fs.existsSync(outputFilePath)) {
      const fileContent = fs.readFileSync(outputFilePath, 'utf8');
      const existingStreetNamesArray = JSON.parse(fileContent);
      existingStreetNamesSet = new Set(existingStreetNamesArray);
    }

    const updatedStreetNamesSet = new Set([...existingStreetNamesSet, ...streetNamesSet]);
    const updatedStreetNamesArray = Array.from(updatedStreetNamesSet);
